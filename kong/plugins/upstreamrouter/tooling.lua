...
function tooling.get_redirect_uri(headers, activation_rules)
  ...
  for key, activation_rule in pairs(activation_rules) do
    ...
    for header, header_value in pairs(activation_rule_headers) do
      if tooling.deepcompare(headers[header], header_value, true) then
        match = match + 1
      end
    end
...
function tooling.deepcompare(t1, t2, ignore_mt)
  local ty1 = type(t1)
  local ty2 = type(t2)
  ...
  for k1, v1 in pairs(t1) do
    local v2 = t2[k1]
    if v2 == nil or not tooling.deepcompare(v1, v2) then return false end
  end
  for k2, v2 in pairs(t2) do
    local v1 = t1[k2]
    if v1 == nil or not tooling.deepcompare(v1, v2) then return false end
  end
  return true
end
